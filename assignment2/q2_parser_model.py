...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_layer = tf.Variable(self.pretrained_embeddings)
        lookup_res = tf.nn.embedding_lookup(embedding_layer, self.input_placeholder)
        embeddings = tf.reshape(lookup_res, (-1, 
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        shape1 = (self.config.n_features * self.config.embed_size, self.config.hidden_size)
        xavier_initializer = xavier_weight_init()
        w = xavier_initializer(shape1)
        b1 = tf.Variable(tf.zeros(self.config.hidden_size,))
        shape2 = (self.config.hidden_size, self.config.n_classes)
        U = xavier_initializer(shape2)
