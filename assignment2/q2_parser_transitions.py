...
def minibatch_parse(sentences, model, batch_size):
    ...
    while True:
        unfinished_parses = [i for i in unfinished_parses if len(i.stack)>1 or len(i.buffer)>0]
        if len(unfinished_parses) == 0:
            break
        tag = False
        batch_data = random.sample(unfinished_parses, min(len(unfinished_parses), batch_size))
        transitions = model.predict(batch_data)
        for parse, transition in zip(batch_data, transitions):
            tem = parse.parse_step(transition)
            tag = tag or tem
        if tag == False:
            break
