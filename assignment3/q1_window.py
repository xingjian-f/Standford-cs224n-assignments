...
def make_windowed_data(data, start, end, window_size = 1):
	...
	### YOUR CODE HERE (5-20 lines)
		for i in xrange(len(sentence)):
			x = []
			y = labels[i]
			nb_start = max(0, window_size-i)
			nb_end = max(0, window_size+i-len(sentence)+1)
			x.extend(nb_start * start)
			for j in sentence[max(0, i-window_size) : min(len(sentence),i+window_size+1)]:
				x.extend(j)
			x.extend(nb_end * end)
...
class WindowModel(NERModel):
	...
	def add_embedding(self):
		...
		lookup_res = tf.nn.embedding_lookup(embed_tensor, self.input_placeholder)													 
